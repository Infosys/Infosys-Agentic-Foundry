...
const GroupAgentAssignment = () => {
  ...
  // Fetch paginated groups from backend using domain endpoints
  const fetchPaginatedGroups = async (pageNumber = 1, pageSize = ITEMS_PER_PAGE, searchTerm = "") => {
    ...
    try {
      ...
      try {
        ...
        const apiUrl = `${APIs.GET_DOMAINS_SEARCH_PAGINATED}?${params.join("&")}`;
        response = await fetchData(apiUrl);
      } catch (paginatedError) {
        // Fallback to GET_DOMAINS if paginated endpoint not available
      ...
      source = Array.isArray(response.details) ? response.details : Array.isArray(response.domains) ? response.domains : Array.isArray(response) ? response : [];
