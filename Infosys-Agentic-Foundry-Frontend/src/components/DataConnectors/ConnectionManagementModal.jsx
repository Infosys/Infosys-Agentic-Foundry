...
const ConnectionManagementModal = ({
  ...
  const { getActivePostgresConnections, getActiveMySQLConnections, getActiveSQLiteConnections, getActiveMongoConnections } = useDatabase();
  ...
  useEffect(() => {
    if (selectedConnection && filteredConnections.length > 0) {
      const connectionData = filteredConnections.find(
        (conn) => conn.connection_name === selectedConnection || conn.name === selectedConnection || conn.id === selectedConnection || conn.connectionName === selectedConnection
      );
  ...
  const handleConnectionSelect = (connectionId) => {
    ...
    const connectionObj = availableConnections.find(
      (conn) => conn.connection_name === connectionId || conn.name === connectionId || conn.id === connectionId || conn.connectionName === connectionId
    );
  ...
  const isSelectedConnectionActive = () => {
    ...
    const connectionName = selectedConnectionData.connection_name || selectedConnectionData.name || selectedConnectionData.id || selectedConnectionData.connectionName;

    const databaseType = (selectedConnectionData.connection_database_type || selectedConnectionData.type || selectedConnectionData.databaseType || "").toLowerCase();
  ...
  return (
    <Modal
      ...
      onClose={onClose}
      size="md"
      ariaLabel="Manage Database Connections"
    ...
    >
      ...
      <div className={styles.modalHeader}>
        ...
        <div className={styles.activeConnectionsContainer}>
        ...
        <div className={styles.modalBody}>
          <div className={styles.formSection}>
            ...
            <div className="formGroup">
              <label className="label-desc">Select Connection:</label>
              <NewCommonDropdown
                options={filteredConnections.map((connection) => connection.connection_name || connection.name || connection.connectionName || connection.title || `Connection`)}
                selected={selectedConnection}
                onSelect={(value) => handleConnectionSelect(value)}
