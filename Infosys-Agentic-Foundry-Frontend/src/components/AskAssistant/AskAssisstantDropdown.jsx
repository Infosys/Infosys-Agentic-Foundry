...
const DropDown = (props) => {
  ...
  useEffect(() => {
    if (highlightIndex >= 0 && isDropdownOpen) {
      const optionEl = document.getElementById(
        `dropdown-option-${highlightIndex}`
      );
  ...
  const displayLabel =
    fullLabel.length > 5 ? `${fullLabel.slice(0, 10)}...` : fullLabel;
  return (
    // <div
    ...
    <>
      ...
      <span
        ...
        onKeyDown={handleKeyDown}
        tabIndex={0}
        className={"agentNodeCss"}
      >
        <span onClick={() => {
          if (!disabled) setIsDropdownOpen((prevState) => !prevState);
        }} className={"chevronCss"} title={fullLabel}>
        ...
        {isDropdownOpen && !disabled && (
          <div
            className={`${styles.dropdownMenu} ${
              isSearch ? styles.dropdownMenuWithSearch : ""
            }`}
          >
