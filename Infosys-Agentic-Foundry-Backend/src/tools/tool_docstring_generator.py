...
def is_syntax_valid(function_string):
    ...
    try:
        ...
        log.info("Function syntax is valid.")
        return True
    except SyntaxError as e:
        log.error(f"Function syntax is invalid: {e}")
        return False
...
def update_docstring(code, new_docstring):
    ...
    if sum(isinstance(node, ast.FunctionDef) for node in tree.body) != 1:
        log.error("Input must be a single function definition")
        raise ValueError('Input must be a single function definition')

    if not isinstance(tree.body[-1], ast.FunctionDef):
        log.error("The last node must be a function definition")
        raise ValueError("There should be no scripts after the function defination")

    for node in tree.body[:-1]:
        if not isinstance(node, (ast.Import, ast.ImportFrom)):
            log.error("Only import statements are allowed in global scope (i.e. only import statements can be used before function defination)")
...
async def generate_docstring_tool_onboarding(llm, tool_code_str, tool_description=""):
    if not is_syntax_valid(tool_code_str):
        try:
            ast.parse(tool_code_str)
            log.error("Tool code syntax is invalid.")
            return f"Tool Onboarding Failed"
        except Exception as e:
            log.error(f"Tool code parsing error: {e}")
