...
class HybridAgentOnboard(BaseAgentOnboard):
    ...
    def __init__(self, agent_service_utils: AgentServiceUtils):
        super().__init__(agent_type=AgentType.HYBRID_AGENT, agent_service_utils=agent_service_utils)
    ...
    async def _generate_system_prompt(self, agent_name: str, agent_goal: str,
        ...
        """
        hybrid_agent_system_prompt_template = PromptTemplate.from_template(hybrid_agent_system_prompt_generator_prompt)
        hybrid_system_prompt_gen = hybrid_agent_system_prompt_template | llm | StrOutputParser()
        log.info(f"Generating Hybrid Agent System Prompt for agent {agent_name}")

        hybrid_agent_system_prompt = await hybrid_system_prompt_gen.ainvoke({
        ...
        return {"SYSTEM_PROMPT_HYBRID_AGENT": hybrid_agent_system_prompt}
