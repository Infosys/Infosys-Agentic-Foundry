...
from src.schemas import PlanSchema, ResponseSchema, CriticSchema, AdminConfigLimits
...
class PlannerExecutorCriticAgentGADKInference(BaseAgentGADKInference):
    """
    Inference class for Planner-Executor-Critic Agent using Google ADK.
    Implements methods to build agents, chains, and workflows specific to the Planner-Executor-Critic architecture.
    """
    ...
    async def _build_agent_and_chains(self, llm: LiteLlm, agent_config: dict, tool_interrupt_flag: bool = False, tools_to_interrupt: Optional[List[str]] = None) -> dict:
        ...
        tools = await self._get_tools_instances(tool_ids=agent_config["TOOLS_INFO"], tool_versions=tool_versions)
        ...
        log.info(f"Building Planner-Executor-Critic Agent and chains for agent: {agent_name}")
        ...
        log.info("Customizing system prompts for Planner-Executor-Critic Agent.")
        ...
        critic_based_planner_system_prompt = system_prompt.get("SYSTEM_PROMPT_CRITIC_BASED_PLANNER_AGENT", "").replace("```json", "").replace("```", "")
...
"""
        ...
        critic_based_planner_system_prompt = f"""\
{critic_based_planner_system_prompt}
...
"""
        ...
        log.info("Creating Planner-Executor-Critic Agent instances.")
        ...
        critic_based_planner_agent = LlmAgent(
            name=f"critic_planner_for_{agent_name}",
            model=llm,
            instruction=critic_based_planner_system_prompt,
            tools=tools,
            output_schema=PlanSchema,
            after_agent_callback=await self.get_after_planner_agent_callback(),
            output_key="plan"
        ...
        chains = {
            ...
            "critic_based_planner_agent": critic_based_planner_agent,
    ...
    def get_general_query_router_decision_function(agent_name: str, quality_assessment_enabled: bool = False) -> Callable:
        ...
        Args:
            ...
            quality_assessment_enabled: If True, routes to executor_quality_loop; if False, routes to executor_critic_loop
        """
        def _general_router_decision_function(state: State) -> str:
            if state.get("plan", []):
                ...
                return f"executor_critic_loop_for_{agent_name}"
    ...
    async def _build_workflow(self, chains: dict, flags: Dict[str, bool] = {}) -> BaseAgent:
        """
        Builds the workflow for the Planner-Executor-Critic agent.
        
        Workflow options:
        1. Basic: Planner -> Executor Loop -> Response Generator -> Critic -> Critic-based Planner
        2. With Quality Assessment: Response Generator is wrapped in quality assessment loop for quality improvement
        ...
        critic_based_planner_agent = chains.get("critic_based_planner_agent", None)
        ...
        if not all([planner_agent, replanner_agent, executor_agent, critic_agent, response_gen_agent, general_query_handler, critic_based_planner_agent, llm]):
            log.error("One or more required chains are missing to build the workflow.")
            raise ValueError("One or more required chains are missing to build the workflow.")
        
        log.info(f"Building workflow for Planner-Executor-Critic Agent. Evaluation enabled: {evaluation_flag}, Validation enabled: {validator_flag}")
        ...
        # When quality_assessment_enabled is True: Quality Assessment REPLACES Critic (routes to evaluation_replanner_agent on failure)
        # When quality_assessment_enabled is False: Use Critic (routes to critic_based_planner_agent on failure)
        if quality_assessment_enabled:
            ...
            # Executor loop with quality assessment (replaces critic)
            ...
            # Executor loop with critic (original flow)
            # Flow: executor_loop_agent -> response_gen_agent -> critic_agent -> critic_based_planner_agent (on failure)
            executor_critic_loop = LoopAgent(
                name=f"executor_critic_loop_for_{agent_name}",
                sub_agents=[executor_loop_agent, response_gen_agent, critic_agent, critic_based_planner_agent],
                max_iterations=inference_config.max_critic_epochs,
                after_agent_callback=await self.get_after_critic_loop_agent_callback()
            )
            log.info("Created executor-critic loop with critic_based_planner_agent for improvement iterations.")
            execution_loop = executor_critic_loop
