...
from inference import segregate_conversation_from_chat_history
...
def build_replanner_executor_critic_chains(llm, multi_agent_config):
...
def build_replanner_executor_critic_agent(llm, chains_and_tools_data: MultiAgentInfernceChains):
...
'''
    ...
    def replanner_agent(state: PlanExecuteCritic): 
        """
...
**Note**:
- Update or revise the current plan according to the user's feedback.
...
'''
        invocation_input = {"messages": [("user", formatted_query)]}
        replanner_response = output_parser(llm=llm, 
...
'''
    ...
    def final_response(state: PlanExecuteCritic):
        ...
        # Generate a primary key for the chat history entry
...
def generate_response_replanner_executor_critic_agent(query,
                                                    multi_agent_config,
                                                    session_id,
                                                    table_name,
                                                    sqlite_identifier,
    ...
    try:
        ...
        chains_and_tools_data = build_replanner_executor_critic_chains(llm=llm, 
                                                                       multi_agent_config=multi_agent_config)
        
        workflow = build_replanner_executor_critic_agent(llm,
        ...
        def invoke(arg, approval, feedback):
            with PostgresSaver.from_conn_string(DATABASE_URL) as checkpointer:
                app = workflow.compile(checkpointer=checkpointer)
                if not approval and not feedback:
                    return app.invoke(arg, config=graph_config)
                elif approval=='yes':
                    return app.invoke(Command(resume='yes'), config=graph_config)
                elif approval=='no' and not feedback:
                    return app.invoke(Command(resume='no'), config=graph_config)
                elif approval=='no' and feedback is not None:
                    return app.invoke(Command(resume=feedback), config=graph_config)
...
def human_in_the_loop_replanner_inference(request: ReplannerAgentInferenceRequest):
    """
    ...
    try:
        ...
        # Construct table and sqlite identifier
        table_name = f'table_{agentic_application_id.replace("-","_")}'
        sqlite_identifier = table_name
        # Retrieve agent tool configuration
        try:
            multi_agent_config = get_agent_tool_config_planner_executor_critic(agentic_application_id=agentic_application_id)
        except Exception as e:
        ...
        response = generate_response_replanner_executor_critic_agent(
            query=query,
            multi_agent_config=multi_agent_config,
            session_id=session_id,
            table_name=table_name,
            sqlite_identifier=sqlite_identifier,
            reset_conversation=reset_conversation,
