...
<div className={styles.accordionContainerToolCall}>
      <div className={styles["accordion-content"]}>
        <div className="MessagingboxToolCall">
          ...
          <div className={styles.planContainer}>
            ...
            <p className={styles.toolcalltextCss}><h3>{"Tool Calls"}</h3></p>
            </span>
            <div className={styles.toolNameCss}>
            <span><h5>{"Tool Name :"}</h5></span>
            <span className={styles.toolNameTextCss}>
             <ReactMarkdown rehypePlugins={[remarkGfm]}>
               {(props?.messageData?.toolcallData&&props?.messageData?.toolcallData?.additional_details[0])?props?.messageData?.toolcallData?.additional_details[0]?.additional_kwargs?.tool_calls[0]?.function?.name:""}
             </ReactMarkdown>
             </span>
             </div>
             <div>
             {props.isEditable ?(
              <>           
               <p className={styles.toolCallCss}><h5>{"Arguments :"}</h5></p>
               {/* <span><h5>{"Arguments:"}</h5></span> */}
           {(props?.messageData?.toolcallData&&props?.messageData?.toolcallData?.additional_details[0])&&props?.messageData?.toolcallData?.additional_details[0]?.additional_kwargs?.tool_calls[0]?.function?.arguments==="{}"?<>
          <span>{"No arguments to show"}</span></>:<>
          <div>
                {Object.entries(props?.parsedValues).map(([key,val])=>(
                    <>
              {typeof val ==='object'||val?.length > 10 ?<>
                        <div className="argumentValueTextArea">
                    <span  className={styles.toolDisplay}><h5>{key}:</h5></span>
                    <textarea className="argumentValueTextArea" value={typeof val==="object"?JSON.stringify(val):val} onChange={(e)=>props.handleEditChange(key,e.target.value,val)}/>
                    </div>
                    </>:<>
                    <div className={styles.argumentinputCss}>
                    <span  className={styles.toolDisplay}><h5>{key}:</h5></span>
              <input className={styles.toolNameTextCss} type="text" value={typeof val==="object"?JSON.stringify(val):val} onChange={(e)=>props.handleEditChange(key,e.target.value,val)} autoFocus/>
              </div>
                    </>}
              </>
                ))}
          ...
          <span>{"No arguments to show"}</span>
           ...
           {/* <ReactMarkdown rehypePlugins={[remarkGfm]}>
             ...
             {props?.messageData?.toolcallData?.additional_details?.[0]&& Object.entries(
  JSON.parse(
  ...
  <div key={key} className={styles.toolNameCss}>
    <h5>{key}:</h5> <p className={styles.toolNameTextCss}>{typeof value === "object" ? JSON.stringify(value) : String(value)}</p>
  </div>
))}
