...
from langchain_core.messages import AIMessage, HumanMessage, ChatMessage, AnyMessage,ToolMessage
...
<<<<<<< HEAD
    delete_by_session_id_from_database, insert_chat_history_in_database,
    worker_agents_config_promptp,get_long_term_memory_from_database
=======
    delete_by_session_id_from_database, insert_chat_history_in_database,
    worker_agents_config_prompt,get_long_term_memory_from_database
>>>>>>> 37f56195f6f7b9d9bb6e69c1c9a279c3e2edf219
)
...
async def build_supervisor_chains(model_name, agentic_application_id):
    llm = load_model(model_name=model_name)
    worker_agents = json.loads(agent_data[agentic_application_id]["tools_id"])
    agent_configs, _, _ = await worker_agents_config_promptp(agentic_app_ids=worker_agents, llm=llm)

    # Load system prompts from your config
    system_prompts = json.loads(agent_data[agentic_application_id]['system_prompt'])
    ...
    # --- Define Chains ---
    # Planner Chain with Pydantic output
    planner_prompt_template = ChatPromptTemplate.from_messages([("system", meta_agent_planner_prompt), ("placeholder", "{messages}")])
    meta_planner_chain_1 = planner_prompt_template | llm | JsonOutputParser()
    meta_planner_chain_2 = planner_prompt_template | llm | StrOutputParser()

    # Response Generator Chain
    responder_prompt_template = ChatPromptTemplate.from_messages([("system", meta_agent_responder_prompt), ("placeholder", "{messages}")])
    meta_responder_chain = responder_prompt_template | llm | StrOutputParser()
    ...
    # Create supervisor workflow for this single step
    supervisor_workflow = create_react_agent(
        model=llm,
        prompt=meta_agent_supervisor_prompt,
        tools=handoff_tools,
    )

    supervisor_agent = supervisor_workflow
    return meta_planner_chain_1, meta_planner_chain_2, meta_responder_chain, conversation_summary_chain, supervisor_agent, agent_configs

async def build_supervisor_graph(conversation_summary_chain, llm, meta_planner_chain_1, meta_planner_chain_2, supervisor_agent, meta_responder_chain):
    # --- Build Graph ---
    ...
    async def meta_planner_agent(state: OverallState):
...
"""
        ...
        try:
            planner_response = await output_parser(
                                llm=llm,
                                chain_1=meta_planner_chain_1,
                                chain_2=meta_planner_chain_2,
    ...
    async def meta_supervisor_executor(state: OverallState):
    ...
    def increment_step(state: OverallState):
        """Increments the step counter."""
        new_idx = state["step_idx"] + 1
        return {"step_idx": new_idx}

    async def meta_response_generator(state: OverallState):
...
"""
    ...
    def route_query(state: OverallState):
    ...
    def check_plan_execution_status(state: OverallState):
...
async def generate_response_meta_agent(query, agentic_application_id, session_id, table_name, model_name, reset_conversation, project_name):
    ...
    (meta_planner_chain_1, 
     meta_planner_chain_2, 
     meta_responder_chain, 
     conversation_summary_chain, 
     supervisor_agent, 
     agent_configs) = await build_supervisor_chains(model_name=model_name, agentic_application_id=agentic_application_id)
    
    workflow = await build_supervisor_graph(
        conversation_summary_chain=conversation_summary_chain,
        llm=load_model(model_name=model_name),
        meta_planner_chain_1=meta_planner_chain_1,
        meta_planner_chain_2=meta_planner_chain_2,
...
async def meta_agent_with_planner_inference(request: AgentInferenceRequest):
    """
