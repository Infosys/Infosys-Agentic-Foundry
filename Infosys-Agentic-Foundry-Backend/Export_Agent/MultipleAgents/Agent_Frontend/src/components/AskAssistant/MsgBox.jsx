...
const MsgBox = (props) => {
  ...
  const {
    ...
    setGenerating,
    oldSessionId,
    session,messageDisable,isEditable,setIsEditable
  } = props;
const[parsedValues,setParsedValues]=useState({})
const rawData = (messageData?.toolcallData&&messageData?.toolcallData?.additional_details[0])?messageData?.toolcallData?.additional_details[0]?.additional_kwargs?.tool_calls[0]?.function?.arguments:""
...
useEffect(()=>{
  ...
  const onMsgEdit=(data)=>{
    const jsonString =(data?.toolcallData&&data?.toolcallData?.additional_details[0])?data?.toolcallData?.additional_details[0]?.additional_kwargs?.tool_calls[0]?.function?.arguments:""
...
let messagetext = messageData?.filter(item=>item.message.trim()==="")
  ...
  return (
    <div>
      ...
      {isShowIsHuman && agentType ==="react_agent" || agentType===REACT_AGENT ?<>
        <div className={`${styles.toogle} ${isTool ? styles.selected : ""}`}>
        <label>Tool Verifier</label>

  {messageDisable || fetching?<>
    <span className={`${styles.toogleData}`}>
    <Toggle  value={toolInterrupt} disabled={messageDisable}/>
    </span>
  </>:<>
  <span className={`${styles.toogleData}`}>
    <Toggle onChange={handleToggle}  value={toolInterrupt} disabled={messageDisable}/>
    </span>
  </>}
      ...
      {isShowIsHuman  && (
          ...
          {selectedOption==="MULTI_AGENT" ||selectedOption==="multi_agent" ?<>
            <span className={`${styles.toogle} ${isHuman ? styles.selected : ""}`}>
                  <label> Plan Verifier</label>
                  {messageDisable || fetching?<>
    <span className={`${styles.toogleData}`}>
    <Toggle  value={isHuman} disabled={messageDisable} />
    </span>
  </>:<>
  <span className={`${styles.toogleData}`}>
  <Toggle onChange={handleToggle} value={isHuman} disabled={messageDisable} />
  </span>
  </>}
                  </span>
                  <span className="">
            <span className={`${styles.toogle} ${isTool ? styles.selected : ""}`}>
                            <label>Tool Verifier</label>
                            {messageDisable || fetching?<>
    <span className={`${styles.toogleData}`}>
    <Toggle  value={toolInterrupt} disabled={messageDisable}/>
  ...
  <span className={`${styles.toogleData}`}>
  <Toggle onChange={handleToggle2} value={toolInterrupt} disabled={messageDisable}/>
  </span>
  </>}
                </span>
                </span>
  ...
  <Toggle onChange={handleToggle2} value={toolInterrupt} disabled={messageDisable}/>
      ...
      ((agentType === REACT_AGENT && close) || agentType === MULTI_AGENT || agentType === PLANNER_META_AGENT)
        ? messageData.slice(-2)
        : messageData
      )?.map((data, index) => {
        const lastIndex =
...
{data.message && agentType !== CUSTOM_TEMPLATE && (agentType === PLANNER_META_AGENT) && (data?.plan?.length==0)
                    ...
                    {index === lastIndex &&
data?.plan?.length > 0 && agentType!==PLANNER_META_AGENT &&
data?.message === "" && ((isHuman&&toolInterrupt)|| isHuman)&&
(
  ...
  (
    ...
    Object.keys(data.toolcallData.additional_details[0]?.additional_kwargs || {}).length === 0
                        ...
                        <>
                          ...
                          {showInput && (
                            <>
                              {(agentType === REACT_AGENT || agentType === MULTI_AGENT) && close && (
                      ...
                      {index === lastIndex &&
data?.plan?.length > 0 &&
data?.message !== "" && agentType === PLANNER_META_AGENT&&
(
  !data?.toolcallData?.additional_details ||  // not present at all
  (
    Array.isArray(data.toolcallData.additional_details) &&
...
{agentType !== CUSTOM_TEMPLATE && (agentType === PLANNER_META_AGENT && !continueButton)  && (
  ...
  Object.keys(data.toolcallData.additional_details[0]?.additional_kwargs).length > 0 )&& (
                        ...
                        <ToolCallFinalResponse
                            ...
                            text={text}
                            argunentKey={argunentKey}
                            parsedValues={parsedValues}setParsedValues={setParsedValues}
                            rawData={rawData}
          ...
          >
                            ...
                            {
        (data?.message === ""  &&(!isHuman && toolInterrupt)) &&
              ...
              <div className={styles["accordion-header"]}>
                <div className="Messagingbox">
                  <div className="table-container">
                    ...
                    {feedBack === dislike &&
                      index === lastIndex &&
                      ((agentType === REACT_AGENT ||agentType === "react_agent") ||(agentType===MULTI_AGENT || agentType==="multi_agent")||(agentType ===PLANNER_META_AGENT)) && (
                        <div className={styles.feedBackSection}>
                          {agentType === REACT_AGENT || agentType=== MULTI_AGENT || (agentType ===PLANNER_META_AGENT && continueButton)&& feedBack === dislike && close && (
                            <div className={styles["cancel-btn"]}>
                              <button
                                onClick={() => {
                                  setClose(false);
                    ...
                    {index === lastIndex &&
                      ((agentType === REACT_AGENT) ||(agentType === MULTI_AGENT) ||(agentType ===PLANNER_META_AGENT && continueButton) )&&
                      feedBack !== dislike && (
  ...
  >
            <SVGIcons icon="fa-solid fa-pen" width={16} height={16} fill={"  #007ac0"} />
          </button>
</>:<>

{
  (data?.message === "" && props?.likeIcon) ||(data?.message === ""&& (data?.plan||!data?.plan))||
  (data?.message === "" && isHuman)&&
  (
  (Array.isArray(data?.toolcallData?.additional_details) &&
    data.toolcallData.additional_details.length > 0 &&
    Object.keys(data.toolcallData.additional_details[0]?.additional_kwargs || {}).length === 0)
