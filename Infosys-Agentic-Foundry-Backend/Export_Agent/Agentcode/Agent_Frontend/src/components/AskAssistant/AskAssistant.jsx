...
const AskAssistant = () => {
  ...
  const [isListening, setIsListening] = useState(false);  const [recording, setRecording] = useState(false);
  ...
  const [cachedSuggestions, setCachedSuggestions] = useState({ user_history: [], agent_history: [] });
  ...
  const sendAudioForTranscription = async (audioBlob) => {
    ...
    try {
      const response = await fetch("http://10.77.82.120:5001/transcribe/", {
        method: "POST",
        body: formData,
      });
  ...
  }, [showKnowledgePopover, showChatHistory, showSettings, showPromptSuggestions]);
  ...
  const sendHumanInLoop = async (isApprove = "", feedBack = "", userText) => {
    ...
    try {
      const url =
        agentType === CUSTOM_TEMPLATE
          ...
          : agentType === PLANNER_EXECUTOR_AGENT
            ...
            : APIs.PLANNER;      response = await postData(url, payload);
      ...
      // Auto-detect and open Canvas for code content
      ...
      const url =
        agentType === META_AGENT
          ...
          : agentType === PLANNER_META_AGENT
            ...
            : APIs.REACT_MULTI_AGENT_QUERY;      const response = await getChatQueryResponse(payload, url);
      setLastResponse(response);
      if (response === null) {
        setShowToast(true);
        setTimeout(() => {
  ...
  const handleKnowledgeDropdownKeyDown = (e) => {
    ...
    const filteredKnowledgeOptions = Array.isArray(knowledgeResponse) 
      ? knowledgeResponse.filter(option => option.toLowerCase().includes(searchTerm.toLowerCase()))
      : [];
  ...
  const [suggestionData, setSuggestionData] = useState({ history: [], recommendations: [] });
  const handleInputChangeForSuggestion = (e) => {
    ...
    setSuggestionData({ history: historyMatches, recommendations: recommendationsMatches });
  ...
  // Auto-detect canvas content from AI responses
  const detectCanvasContent = (message) => {
    ...
    try {
      if (typeof message === 'string') {
        ...
        // Try to extract text content from objects
        if (message.text) {
                    ...
                    <div className={chatInputModule.relativeWrapper} ref={knowledgePopoverRef}>
                      ...
                      {showKnowledgePopover && (
                        <div className={chatInputModule.dropdownContent} role="listbox" aria-label="Knowledge Base options" style={{ minWidth: 220, maxWidth: 260 }}>
                          ...
                          <div className={chatInputModule.agentsList} ref={knowledgeListRef} style={{ maxHeight: 240, overflowY: 'auto', padding: '4px 8px' }}>
