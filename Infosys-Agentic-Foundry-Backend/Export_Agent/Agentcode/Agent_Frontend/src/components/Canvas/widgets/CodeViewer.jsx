...
const CodeViewer = ({ content, messageId, language = 'javascript', theme = 'light' }) => {
  ...
  const detectLanguage = (code) => {
    ...
    // Simple language detection based on common patterns - check Python first
    if (codeStr.includes('def ') || codeStr.includes('import ') || 
        codeStr.includes('from ') || codeStr.includes('print(') ||
        codeStr.includes('if __name__') || codeStr.includes('class ') ||
        /for\s+\w+\s+in\s+/.test(codeStr) || /^\s*#/.test(codeStr)) return 'python';
    
    if (codeStr.includes('function ') || codeStr.includes('=>') || 
        codeStr.includes('const ') || codeStr.includes('let ') || 
        codeStr.includes('var ') || codeStr.includes('console.')) return 'javascript';
    
    if (codeStr.includes('SELECT ') || codeStr.includes('FROM ')) return 'sql';
    if (codeStr.includes('<html') || codeStr.includes('<!DOCTYPE')) return 'html';
    if (codeStr.includes('<?php')) return 'php';
    if (codeStr.includes('#include') || codeStr.includes('int main')) return 'cpp';
    if (codeStr.includes('public class') || codeStr.includes('import java')) return 'java';
    if (codeStr.includes('package ') && codeStr.includes('func ')) return 'go';
    
    return language || 'text';
  };

  const detectedLanguage = detectLanguage(safeContent);
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `code-${messageId || Date.now()}.${getFileExtension(detectedLanguage)}`;
