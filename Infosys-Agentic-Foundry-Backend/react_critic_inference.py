...
async def build_executor_critic_chains(llm, multi_agent_config,checkpointer):
    ...
    log.info(" Executor, Critic chains built successfully.")
    return data

async def build_executor_critic_agent(llm, chains_and_tools_data: MultiAgentInfernceChains,interrupt_flag=False):
...
#         print(state["is_interrupted"])
#         if state["is_interrupted"]:
#             print("inside of If of executor agent")
#             executor_agent_response = await agent_executor.ainvoke(None,thread)
#         else:
#             print("inside of Else of executor agent")
...
"""
        ...
        #log.info(f"Executor agent executed step {state['step_idx']} for session {state['session_id']}")
        log.info(f"Executor Agent Final response stored successfully")
        return {
...
'''
    ...
    def critic_decision(state: PlanExecuteCritic):
        ...
        if state["response_quality_score"]>=0.7 or state["epoch"]==3:
            ...
            return "executor_agent" #Command(goto="critic_based_planner_agent")
    ...
    async def final_decision(state: PlanExecuteCritic):
        thread = {"configurable": {"thread_id": "inside"+f"{state['table_name']}_{state['session_id']}"}}
        # print(agent_executor.get_state(thread).values["messages"][-1])
        a = await agent_executor.aget_state(thread)
        if a.tasks == ():
            print("No tasks found in the agent executor state.")
...
async def generate_response_executor_critic_agent(query,
                                                    multi_agent_config,
                                                    session_id,
                                                    table_name,
                                                    db_identifier,
    ...
    async with AsyncPostgresSaver.from_conn_string(DB_URI) as checkpointer:
        try:
            ...
            chains_and_tools_data = await build_executor_critic_chains(llm=llm,
                                                                        multi_agent_config=multi_agent_config,checkpointer=checkpointer,)

            workflow = await build_executor_critic_agent(llm,
